/* L'en-tête d'une boutique : nom sur une ligne, slogan dessous. */
...
const mesure = (page) => page.evaluate(() => {
  ...
  const v = document.querySelector(".topbar-vignette");
  ...
  return {
    nom: h ? h.textContent : "",
    nomCoupe: h ? h.scrollWidth > h.clientWidth + 1 : false,
    lignesNom: ligne(h),
...
for (const [l, quoi] of [[360,"un petit téléphone (360)"],[390,"un téléphone courant (390)"]]) {
  ...
  ok(m.lignesNom === 1, "sur une seule ligne (" + m.lignesNom + ")");
  ok(m.largeurNom > 200,
    "il dispose de la largeur qu'il faut (" + m.largeurNom + " px, contre ~120 avant)");
  ok(m.lignesSlogan === 1, "le slogan tient sur une ligne (" + m.lignesSlogan + ")");
  ...
  /* L'ancien en-tête faisait ~140 px avec son slogan sur quatre
     lignes : le nouveau doit être PLUS COURT, pas plus haut. */
...
titre("Un slogan très long ne déborde toujours pas");
{
  ...
  const m = await mesure(page);
  ok(m.lignesSlogan === 1, "il reste sur une ligne (" + m.lignesSlogan + ")");
...
titre("L'accueil de BIZZOO n'a pas bougé");
{
  ...
  ok(!t.enseigne, "et l'en-tête de boutique ne s'y invite pas");
  await ctx.close();
}
